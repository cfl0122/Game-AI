...
class Sequence(BTNode):
	...
	def execute(self, delta = 0):
		...
		# print "i am in sequence"
		treeLength = BTNode.getNumChildren(self)
		if(treeLength==0):
			return True
		else:
			currentIndex = BTNode.getCurrentIndex(self)
			currentBT = BTNode.getChild(self,currentIndex)
			childReturn = currentBT.execute()
			if(childReturn==False):
				return False
			elif(childReturn==None):
				return None
			elif(childReturn==True):
				currentIndex+=1
				if(currentIndex<treeLength):
					BTNode.setCurrentIndex(self,currentIndex)
					return None
		return True
