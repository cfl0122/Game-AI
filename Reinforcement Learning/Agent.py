...
class Agent:
	...
	def updateVtable(self, newState, lastState, action, reward, terminal, availableActions):
		...
		old_value = self.v_table[lastState][action]
		a_t = self.learningRate
		# print "learningRate: ", a_t
		if not terminal:
			maxQ_i = availableActions[0]
			maxQ_value = self.v_table[newState][maxQ_i]
			for i in range(len(availableActions)):
				currentV = self.v_table[newState][availableActions[i]]
				if(currentV>maxQ_value):
					maxQ_value=currentV
					maxQ_i = i
			futureValue = self.v_table[newState][maxQ_i]
			updateValue = old_value+a_t*((reward+self.gamma*(futureValue))-old_value)
			self.v_table[lastState][action]=updateValue
		else:
			updateValue = old_value+a_t*(reward-old_value)
			self.v_table[lastState][action]=updateValue
	...
	def egreedy(self, observation):
		# YOUR CODE GOES BELOW HERE
		avi_action = observation.availableActions
		highestAction = avi_action[0]
		flatState = self.calculateFlatState(observation.worldState)
		highestScore = self.v_table[flatState][highestAction]
		for poss_act in avi_action:
			currentScore = self.v_table[flatState][poss_act]
			if(currentScore>highestScore):
				highestScore = currentScore
				highestAction = poss_act
		randomNum = random.random()
		if(randomNum<0.5):
			# print "i am random"
			return random.randint(0,len(avi_action)-1)
